...
int _write(int file, char* data, int len) {
  ...
  if (CDC_Transmit_FS(&UartHandle, data, len)!= HAL_OK) {
    Error_Handler();
  }
...
int main(void)
{
  ...
  while (1) {
    ...
    if (HAL_UART_Receive_IT(&huart2, uartBuf, 16) != HAL_OK) Error_Handler();
    // Wait for end of transfer
    while (UartReady != SET);
    // Reset transfer flag
    UartReady = RESET;
    // Send it back (While the UART in reception process, user can transmit data through the uartBuf buffer)
    if (HAL_UART_Transmit_IT(&UartHandle, uartBuf, 16)!= HAL_OK) Error_Handler();
    while (UartReady != SET);
    UartReady = RESET;
  /* USER CODE END 3 */
}
